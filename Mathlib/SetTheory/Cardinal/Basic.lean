...
theorem no_max : ∀ ℵ : Cardinal, ¬ IsMax ℵ := by
  intro ℵ h
  unfold IsMax at h; specialize h (le_of_lt $ cantor ℵ)
  exact not_lt_of_le h (cantor ℵ)

theorem succ_lt : ∀ ℵ : Cardinal, ℵ < succ ℵ := λ ℵ => Order.lt_succ_of_not_isMax (no_max ℵ)
