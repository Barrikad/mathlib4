...
This file sets up the ordinal approximation theory of fixed points of a monotone function in a complete lattice.
## Main definitions
* `OrdinalApprox.lfp_approx`: The ordinal approximation of the least fixed point of a bundled monotone function.
* `OrdinalApprox.gfp_approx`: The ordinal approximation of the greatest fixed point of a bundled monotone function.
## Main theorems
* `OrdinalApprox.lfp_is_lfp_approx`: The approximation of the least fixed point eventually reaches the least fixed point
* `OrdinalApprox.gfp_is_gfp_approx`: The approximation of the greatest fixed point eventually reaches the greatest fixed point
## Tags
...
lemma mk_initialSeg_Subtype (ℵ : Cardinal ):
  #{ i : Ordinal // i < (ord  ℵ)} = lift.{u+1, u} ℵ := by
...
lemma lfp_approx_def (a : Ordinal.{u}): lfp_approx f a = sSup { f (lfp_approx f b) | (b : Ordinal) (_ : b < a) } := by
  conv => left; unfold lfp_approx

lemma lfp_approx_addition (a : Ordinal.{u}) :
  f (lfp_approx f a) = lfp_approx f (a+1) := by
  apply le_antisymm
...
lemma lfp_approx_one_fixedPoint (a b : Ordinal.{u}) (h : a < b) (h_fab : lfp_approx f a = lfp_approx f b):
    lfp_approx f a ∈ (fixedPoints f) := by
  rw[mem_fixedPoints_iff,lfp_approx_addition]
  apply Monotone.monotone_stabilizing (lfp_approx f) (lfp_approx_monotone f) h_fab (a+1) (SuccOrder.succ_le_of_lt h) (SuccOrder.le_succ a)

lemma lfp_approx_many_fixedPoints (a b : Ordinal.{u}) (h : a < b) (h_fab : lfp_approx f a = lfp_approx f b):
...
lemma lfp_approx_has_fixedPoint_cardinal : lfp_approx f (ord $ succ #α) ∈ (fixedPoints f) := by
  ...
  | inl h_ab =>
    exact lfp_approx_many_fixedPoints f a b (Ne.lt_of_le h_nab h_ab) h_fab (ord $ succ #α) (le_of_lt h_a)
  | inr h_ba =>
    exact lfp_approx_many_fixedPoints f b a (Ne.lt_of_le (id (Ne.symm h_nab)) h_ba) (h_fab.symm) (ord $ succ #α) (le_of_lt h_b)
...
theorem lfp_is_lfp_approx_cardinal : lfp_approx f (ord $ succ #α) = lfp f := by
  ...
  . have h_fix : ∃ x: fixedPoints f, lfp_approx f (ord $ succ #α) = x := by {simpa using lfp_approx_has_fixedPoint_cardinal f}
    let ⟨x, h_x⟩ := h_fix; rw[h_x]
    exact lfp_le_fixed f x.prop

/-- **Constructive Knaster-Tarski Theorem**: Some ordinal approximation of the least fixed point is the least fixed point.
...
lemma gfp_approx_def (a : Ordinal.{u}): gfp_approx f a = sInf { f (gfp_approx f b) | (b : Ordinal) (_ : b < a) } := by
  conv => left; unfold gfp_approx

lemma gfp_approx_addition (a : Ordinal.{u}) :
  f (gfp_approx f a) = gfp_approx f (a+1) := by
  apply le_antisymm
...
lemma gfp_approx_one_fixedPoint (a b : Ordinal.{u}) (h : a < b) (h_fab : gfp_approx f a = gfp_approx f b):
    gfp_approx f a ∈ (fixedPoints f) := by
  rw[mem_fixedPoints_iff,gfp_approx_addition]
  apply Monotone.antitone_stabilizing (gfp_approx f) (gfp_approx_antitone f) h_fab (a+1) (SuccOrder.succ_le_of_lt h) (SuccOrder.le_succ a)

lemma gfp_approx_many_fixedPoints (a b : Ordinal.{u}) (h : a < b) (h_fab : gfp_approx f a = gfp_approx f b):
...
lemma gfp_approx_has_fixedPoint_cardinal : gfp_approx f (ord $ succ #α) ∈ (fixedPoints f) := by
  ...
  cases (le_total a b) with
  | inl h_ab =>
    exact gfp_approx_many_fixedPoints f a b (Ne.lt_of_le h_nab h_ab) h_fab (ord $ succ #α) (le_of_lt h_a)
  | inr h_ba =>
    exact gfp_approx_many_fixedPoints f b a (Ne.lt_of_le (id (Ne.symm h_nab)) h_ba) (h_fab.symm) (ord $ succ #α) (le_of_lt h_b)
...
theorem gfp_is_gfp_approx_cardinal : gfp_approx f (ord $ succ #α) = gfp f := by
  apply le_antisymm
  . have h_fix : ∃ x: fixedPoints f, gfp_approx f (ord $ succ #α) = x := by {simpa using gfp_approx_has_fixedPoint_cardinal f}
    let ⟨x, h_x⟩ := h_fix; rw[h_x]
...
/-- **Dual Constructive Knaster-Tarski Theorem**: Some ordinal approximation of the greatest fixed point is the greatest fixed point.
